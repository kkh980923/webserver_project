...
<table class="table">
  ...
  <tbody>
    ...
    if($search)
    {
      $argment['search'] = "%$search%";
      $sql = "SELECT COUNT(title) FROM posts WHERE title LIKE :search";
      $count = pdo($pdo, $sql,$argment)->fetchColumn();
      ...
      if($count > 0)
      {
        ...
        $sql = "SELECT * FROM posts WHERE title LIKE :search ORDER BY id DESC LIMIT :show OFFSET :from;";
 ...
 <nav aria-label="Page navigation">
  <ul class="pagination justify-content-center">
    ...
    <li class="page-item"><a href="?search=<?= $search ?>&show=<?= $show ?>&from=<?= (($i - 1) * $show) ?>" class="page-link <?= ($i == $current_page)? 'active" aria-current="true' : '' ?>">
    <?= $i ?>
  </a></li>
  <?php } ?>
  </ul>
